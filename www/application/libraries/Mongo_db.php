...
class Mongo_db
{
	...
	*
	* <code>
	* $this->mongo_db->aggregate($collection = '', array $pipeline(array $op [, array $... ]) );
	* </code>
	*
	* @param string $collection Name of the collection
	* @param string $pipeline An array of mongo pipeline operations
	*
	* @author loyalj
	* @access public
	* @return int
	*/
	public function aggregate($collection = '', $pipeline = array())
	{
		if (empty($collection))
		{
			$this->_show_error('In order to aggregate documents from MongoDB, a collection name must be passed', 500);
		}

                if (empty($pipeline))
		{
			$this->_show_error('In order to aggregate documents from MongoDB, the pipeline must contain operations', 500);
		}

		$aggregate = $this->_dbhandle
						->{$collection}
						->aggregate($pipeline);
		
		$this->_clear($collection, 'aggregate');
		return $aggregate;
	}
